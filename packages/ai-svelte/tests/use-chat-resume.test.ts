...
describe('createChat resume surface', () => {
  it('hydrates resumeState and pendingInterrupts from initialResumeSnapshot', () => {
    ...
    expect(chat.resumeState).toEqual({ threadId: 'thread-1', runId: 'run-1' })
    expect(chat.pendingInterrupts).toEqual([
  ...
  it('resumeInterrupts forwards resume items to the client and clears pending interrupts', async () => {
    ...
    await chat.resumeInterrupts(resumeItems)
    ...
    expect(chat.pendingInterrupts).toEqual([])
    expect(chat.resumeState).toBeNull()
  })
  ...
  it('updates resumeState and pendingInterrupts reactively via onResumeStateChange', async () => {
    ...
    const chat = createChat({ connection: adapter, live: true })

    // No sendMessage() wrapper call here — the update must arrive purely
    // through the client's onResumeStateChange callback into the rune.
    await waitFor(() => {
      expect(chat.resumeState).toEqual({
        threadId: 'thread-live',
        runId: 'run-live-interrupt',
      })
      expect(chat.pendingInterrupts).toEqual([
