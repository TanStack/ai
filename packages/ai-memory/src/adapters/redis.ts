...
export function nodeRedisAsRedisLike(client: NodeRedisLike): RedisLike {
  return {
    ...
    scan: async (cursor, ...args) => {
      ...
      // v5 typed `cursor: string`; `NodeRedisLike.scan` widens both ends to
      // `number | string` so either client's cursor threads through without a
      // lossy `Number(cursor)` coercion (which would drop precision for v5
      // cursors larger than `Number.MAX_SAFE_INTEGER`).
