...
export function createChatUIContexts() {
  ...
  function useInterruptContext() {
    const interrupt = inject(interruptKey)
    if (!interrupt) {
      throw new Error('`useInterruptContext` must be used within UIInterrupt.')
    }
    return interrupt
  }
  return {
