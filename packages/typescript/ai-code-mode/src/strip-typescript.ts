...
export async function stripTypeScript(code: string): Promise<string> {
  try {
    ...
    if (functionStart === -1) {
      console.warn('[stripTypeScript] Could not find wrapper function start')
      return code
    }
    ...
    if (openBrace === -1) {
      console.warn('[stripTypeScript] Could not find opening brace')
      return code
    }
    ...
    if (endMarkerIndex === -1) {
      console.warn('[stripTypeScript] Could not find end marker')
      return code
    }
    ...
    if (closingBraceIndex === -1) {
      console.warn('[stripTypeScript] Could not find closing brace')
      return code
    }
    ...
    // If esbuild fails (e.g., syntax error), return original code
    // The sandbox will then report the actual error
    console.warn('Failed to strip TypeScript from code:', error)
    return code
  }
}
