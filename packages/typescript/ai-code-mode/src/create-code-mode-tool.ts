...
const executeTypescriptInputSchema = z.object({
  typescriptCode: z.string().describe(
...
export function createCodeModeTool(
...
): ServerTool<typeof executeTypescriptInputSchema, typeof executeTypescriptOutputSchema, 'execute_typescript'> {
  const {
    driver,
    tools,
    timeout = 30000,
  ...
  return definition.server(async (input, toolContext?: ToolExecutionContext): Promise<CodeModeToolResult> => {
...
function buildToolDescription(tools: Array<CodeModeTool>): string {
  const externalFunctions = tools
    .map((t) => `external_${t.name}`)
    .join(', ')
