 ...
 */
function makeGrokStructuredOutputCompatible(
...
/**
 * Converts a Zod schema to JSON Schema format compatible with Grok's structured output.
 *
 * Grok's structured output (via OpenAI-compatible API) has strict requirements:
 * - All properties must be in the `required` array
 * - Optional fields should have null added to their type union
 * - additionalProperties must be false for all objects
 *
 * @param schema - Zod schema to convert
 * @returns JSON Schema object compatible with Grok's structured output API
 *
 * @example
 * ```typescript
 * import { z } from 'zod';
 ...
 * });
 *
 * const jsonSchema = convertZodToGrokSchema(zodSchema);
 * // Returns:
 * // {
 * //   type: 'object',
 * //   properties: {
 ...
 * ```
 */
export function convertZodToGrokSchema(schema: z.ZodType): Record<string, any> {
  ...
  if (typeof result === 'object') {
    ...
    // Apply Grok-specific transformations for structured output
    result = makeGrokStructuredOutputCompatible(result, result.required || [])
  }
