...
import { convertMessagesToInput } from './text/text-provider-options'
    ...
    const response = await this.client.chat.completions.create(
      {
        ...
        messages,
        tools,
        tool_choice: options.options?.toolChoice,
    ...
    try {
      for await (const chunk of stream) {
        ...
        if (chunk.type) {
          ...
          if (count === 0) {
            console.log('[OpenAI Adapter] New event type detected:', chunk.type)
          }
        }
