...
/**
 * Generates a unique ID with a prefix for tracing and debugging.
 */
export function generateIdFor(prefix: string, provider: string): string {
    return `${provider}-${prefix}-${Date.now()}-${Math.random().toString(36).substring(7)}`
}
