...
import type { DrizzleDb, DrizzleTransactionExecutor } from '../src/index'
...
function createVariantDb(): {
  ...
  return { db, sqlite, interrupts: createTransactionExecutor(sqlite) }
}
...
runPersistenceConformance('drizzle-sqlite (injected variant schema)', () => {
  const { db, interrupts } = createVariantDb()
  return drizzlePersistence(db, { schema: variantSchema, interrupts })
})

describe('drizzlePersistence with an injected schema', () => {
  it('writes through the injected table and column names', async () => {
    const { db, interrupts, sqlite } = createVariantDb()
