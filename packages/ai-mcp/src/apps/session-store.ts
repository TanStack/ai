...
export interface McpSessionStore {
  ...
  get: (threadId: string, serverId: string) => Promise<McpServerDescriptor | null>
 ...
 * with no API change.
 */
export function inMemoryMcpSessionStore(opts: { ttlMs?: number } = {}): McpSessionStore {
  const map = new Map<string, { at: number; servers: Record<string, McpServerDescriptor> }>()
