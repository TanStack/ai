...
describe('defineByok memory', () => {
  ...
  it('clear removes one key or all keys', async () => {
    ...
    expect(byok.getSnapshot().status.openai.state).toBe('empty')
    expect(byok.getSnapshot().status.grok.state).toBe('set')
    await byok.clear()
    expect(byok.getSnapshot().status.grok.state).toBe('empty')
  })
...
describe('defineByok unlockable update/clear', () => {
  function unlockableStorage(save: ReturnType<typeof vi.fn>): KeyringStorage {
    return {
      id: 'mock-passkey',
      label: 'Mock passkey',
      persistent: true,
