...
function unwrapExec(command: string): string {
  const prefix = "cd '/workspace/home' && "
...
describe('UpstashBoxHandle.fs.lstat', () => {
  it('parses file, directory, symlink, and other metadata', async () => {
    ...
    const { box } = fakeBox({
      exec: (cmd) => {
        ...
        const path = lstatPath(inner)
        expect(inner).toBe(lstatCommand(path))
        return {
        ...
        expect(inner).toBe(
          lstatCommand(
            path.startsWith('/workspace')
              ? `/workspace/home${path.slice('/workspace'.length)}`
              : path,
          ),
        )
        return {
  ...
  it.each([
    ...
    const { box } = fakeBox({
      exec: (cmd) => {
        const inner = unwrapExec(cmd)
        expect(inner).toBe(
          lstatCommand(`/workspace/home${path.slice('/workspace'.length)}`),
        )
