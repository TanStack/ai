import type { AcpPermissionMode as AntigravityCliPermissionMode } from '@tanstack/ai-acp'

/**
 * Per-call provider options for the Antigravity CLI adapter, passed via
 * `modelOptions` on `chat()`.
 */
export interface AntigravityCliTextProviderOptions {
  /**
   * Resume an existing Antigravity CLI session. The adapter emits the session id
   * of every run via a CUSTOM `antigravity-cli.session-id` stream event; thread
