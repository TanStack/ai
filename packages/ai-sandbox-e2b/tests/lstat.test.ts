...
import { E2BHandle } from '../src/handle'
...
function createHandle(): E2BHandle {
  return new E2BHandle({
    sandbox: { sandboxId: 'test' } as unknown as Sandbox,
    workdir: '/workspace',
  })
}

describe('E2BHandle.fs.lstat', () => {
  ...
  ])('rejects malformed lstat fields: size=%s mode=%s', async (size, mode) => {
    const handle = createHandle()
    execSlot(handle).exec = async () => ({
      exitCode: 0,
      stdout: `${mode}:${size}\n`,
  ...
  it('preserves an unverified parent failure', async () => {
    const handle = createHandle()
    execSlot(handle).exec = async () => ({
      exitCode: 1,
      stdout: '',
    ...
    await expect(handle.fs.lstat!('/workspace/file/child')).rejects.toThrow(
