...
describe('Prisma persistence CLI', () => {
  it('writes the default Prisma migration path', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'tanstack-ai-prisma-'))
    ...
    expect(result).toEqual({
      ...
      path: join(
        ...
        'prisma',
        'migrations',
        '20260706123456_tanstack_ai_persistence',
        'migration.sql',
      ),
    })
    if (result.kind === 'file') {
