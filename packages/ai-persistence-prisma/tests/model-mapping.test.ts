...
async function makeRenamedClient(): Promise<PrismaClient> {
  ...
  const aliases = (client: PrismaClient | Prisma.TransactionClient) => ({
    chatMessage: client.message,
    chatRun: client.run,
    chatInterrupt: client.interrupt,
    chatInterruptBatch: client.interruptBatch,
    chatMetadata: client.metadata,
    chatArtifact: client.artifact,
    chatBlob: client.blob,
  })
  const renamed = {
    ...aliases(prisma),
    $transaction: <T>(
      operation: (transaction: ReturnType<typeof aliases>) => Promise<T>,
    ) => prisma.$transaction((transaction) => operation(aliases(transaction))),
  }
...
const renamedModels = {
  ...
  interruptBatches: 'chatInterruptBatch',
...
describe('prismaPersistence model mapping', () => {
  ...
  it('lists every unresolved model in one error', async () => {
    ...
    expect(() => prismaPersistence(prisma)).toThrow(
      /messages[\s\S]*runs[\s\S]*interrupts[\s\S]*interruptBatches[\s\S]*metadata[\s\S]*artifacts[\s\S]*blobs/,
    )
  })
