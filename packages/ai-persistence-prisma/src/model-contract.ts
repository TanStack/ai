...
export interface InterruptBatchRow {
...
export interface InterruptDelegate {
  ...
  findMany: (args: {
    ...
    orderBy: Array<{ requestedAt: 'asc' } | { interruptId: 'asc' }>
...
export interface InterruptBatchDelegate {
  findUnique: (args: {
    where: { interruptedRunId: string }
  }) => Promise<InterruptBatchRow | null>
  create: (args: {
    data: {
      interruptedRunId: string
      threadId: string
...
export interface TanstackAiDelegates {
  ...
  interruptBatch: InterruptBatchDelegate
...
const defaultModelNames: Required<PrismaModelMap> = {
  ...
  interruptBatches: 'interruptBatch',
  metadata: 'metadata',
  artifacts: 'artifact',
  blobs: 'blob',
}
...
/**
 * Resolve the seven model delegates off the client, applying any model-name
