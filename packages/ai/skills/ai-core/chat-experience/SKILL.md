...
- See also: **ai-core/plugin/SKILL.md** -- To bundle chat with other
  app-defined plugins (image generation, narration, other one-shot
  activities) behind a single `definePlugin` handler + `usePlugin` hook
  instead of wiring up `chat()` and `useChat` alone
