// packages/ai/tests/middlewares/memory.test.ts
...
function fakeAdapter(seed: Array<MemoryRecord> = []): MemoryAdapter & {
  ...
  const searchCalls: Array<MemoryQuery> = []
  return {
    name: 'fake',
    store,
    searchCalls,
    ...
    async list(scope, options): Promise<MemoryListResult> {
      const items: Array<MemoryRecord> = []
      for (const r of store.values()) {
        let match = true
        for (const k of Object.keys(scope) as Array<keyof MemoryScope>) {
...
describe('memoryMiddleware — retrieval', () => {
  ...
  it('retrieves at init and injects a memory system prompt', async () => {
    ...
    await collectChunks(stream as AsyncIterable<StreamChunk>)
    const first = calls[0] as { systemPrompts?: Array<string> }
  ...
  it('does not re-inject across agent-loop iterations', async () => {
    ...
    await collectChunks(stream as AsyncIterable<StreamChunk>)
    const iter1 =
      (calls[0] as { systemPrompts?: Array<string> }).systemPrompts?.length ?? 0
    const iter2 =
      (calls[1] as { systemPrompts?: Array<string> }).systemPrompts?.length ?? 0
  ...
  it('calls rerank between search and render', async () => {
    ...
    const rerank = vi.fn(async (hits: Array<MemoryHit>) => [...hits].reverse())
    const stream = chat({
      adapter,
      messages: [{ role: 'user', content: 'hi' }],
      middleware: [
    ...
    const promptText = (
      calls[0] as { systemPrompts: Array<string> }
    ).systemPrompts.join('\n')
