...
async function injectWorkspaceSecrets(
  handle: SandboxHandle,
  workspace: WorkspaceDefinition,
): Promise<void> {
...
async function writeAgentsAndFileSkills(
  handle: SandboxHandle,
  workspace: WorkspaceDefinition,
  root: string,
): Promise<void> {
...
function setupFailureMessage(
  ...
  return `setup step failed: ${command} (exit ${exitCode})${tail ? `\n${tail}` : ''}`
}

async function runSerialSetup(
  shell: Awaited<ReturnType<typeof createBootstrapShell>>,
  command: string,
  ranSetup: Array<string>,
): Promise<void> {
  ...
  if (result.exitCode !== 0) {
    ...
    throw new Error(setupFailureMessage(command, result.exitCode, tail))
  }
...
async function runParallelSetup(
  ...
  if (failed !== undefined) {
    const tail = `${failed.res.stdout}\n${failed.res.stderr}`
      ...
      .slice(-1500)
    throw new Error(
      setupFailureMessage(failed.command, failed.res.exitCode, tail),
    )
  }
  ranSetup.push(...commands)
}
  ...
  try {
    for (const group of plan) {
      if (group.kind === 'serial') {
        await runSerialSetup(shell, group.command, ranSetup)
      } else {
        await runParallelSetup(handle, shell, group.commands, ranSetup, signal)
      }
    }
  } finally {
    await shell.dispose()
...
export async function bootstrapWorkspace(
  ...
  await injectWorkspaceSecrets(handle, workspace)
  await landGitSource(handle, workspace, root)
  await cloneGitSkills(handle, workspace, root)
  await writeAgentsAndFileSkills(handle, workspace, root)
  const packageManager = await detectPackageManager(handle, workspace, root)
  const ranSetup = await runWorkspaceSetup(
    handle,
    workspace,
    root,
    options.signal,
