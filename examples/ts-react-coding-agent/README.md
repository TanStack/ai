...
via `@tanstack/ai-codex`, and [OpenCode](https://opencode.ai) via
...
- **Session resume** — the server emits the harness session id via a
  ...
  `codex.session-id`, `opencode.session-id`); the
...
- **Harness tool timeline** — built-in tools (Read, Grep, Edit,
  ...
  OpenCode stream token-by-token.
- **Permission modes** — a Read-only/Edit toggle maps to each harness's
  ...
  the default-deny vs `acceptEdits` permission policy for OpenCode. With
  Claude Code and OpenCode, ask it to run a shell command and watch the
...
The codex binary is spawned per turn, so the CLI must be on `PATH`.
