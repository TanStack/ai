...
// The `/api/run` route reads ANTHROPIC_API_KEY / DAYTONA_API_KEY from
...
// itself is REMOTE (Daytona's cloud), so there is no host-side tool-bridge or
// preview proxy to configure — the route just talks to the Daytona API and hands
// the agent a pre-minted public preview URL.
//
// `@daytona/sdk` is a server-only dependency used exclusively in the `/api/run`
// route. Keep it out of the client bundle and the dev-time dep pre-bundler:
