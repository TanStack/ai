/**
 * The sandbox agent — Claude Code running inside a **Daytona** cloud sandbox.
 *
 ...
 * - On a hosted provider the sandbox runs in Daytona's cloud and CANNOT reach your
 *   laptop's `localhost`, so there is **no host-tool bridge** here. Instead:
 ...
 *     2. The preview URL is **deterministic and host-resolvable** — Daytona's
 *        `getPreviewLink(port)` returns a public URL — so the host resolves it
 *        up front (see {@link resolvePreviewUrl}) and tells the agent to share it
 *        once the dev server is listening. No callback into the host required.
 *
 ...
 * model) and `DAYTONA_API_KEY` (to create sandboxes).
 */
...
/**
 * The ONE dev-server port the agent's app must bind. `getPreviewLink(PREVIEW_PORT)`
 * opens it and returns the public URL, so the agent MUST use exactly this port.
 */
...
/**
 * System-prompt guidance for the preview. Because the sandbox is hosted in
 * Daytona's cloud (it can't reach your machine), the host has ALREADY minted the
 * public preview URL for {@link PREVIEW_PORT}; the agent just binds the dev server
 * wide on that port and shares the URL — no callback into the host.
 */
...
/**
 * One Daytona sandbox per thread (so a follow-up message resumes the same
 * workspace). The default Daytona snapshot already ships the `claude` CLI, so we
 * only install it as a fallback when it's missing (the `daytona` user can't write
 * to the global npm prefix, so a forced `-g` install would fail — rely on the
 * pre-installed binary). `ANTHROPIC_API_KEY` is injected as a secret for it.
 */
export const sandbox: SandboxDefinition = defineSandbox({
  id: 'sandbox-agent-claude-code-daytona',
  ...
  workspace: defineWorkspace({
    ...
    source: { type: 'none' },
    setup: ({ serial }) => {
      // Use the snapshot's pre-installed `claude`; only install if it's absent.
      serial(
 ...
 * HOST-side call (`handle.ports.connect` → Daytona `getPreviewLink`), and on a
