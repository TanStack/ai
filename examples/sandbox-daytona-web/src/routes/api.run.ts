 ...
 * Daytona sandbox; the adapter spawns `claude` inside it and streams its events
 ...
 * the sandbox is in Daytona's cloud and can't reach this host. The scaffolding
 * recipe is inlined into the system prompt, and the public preview URL is
 * resolved host-side up front and handed to the agent to share.
 */
      ...
      POST: async ({ request }) => {
        ...
        if (!process.env.DAYTONA_API_KEY) {
          return new Response(
            JSON.stringify({
              error:
                'DAYTONA_API_KEY is not set. Get one at https://app.daytona.io/ , export it, and restart.',
            }),
            { status: 500, headers: { 'content-type': 'application/json' } },
          )
        }
        ...
        try {
          ...
          const stream = chat({
            ...
            tools: [],
            // Resume-or-create the thread's Daytona sandbox and provide the handle
