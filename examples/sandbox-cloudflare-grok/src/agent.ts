 ...
 * the `grok` CLI. The package also supports a `colocated` mode (the harness loop
 ...
 *
 * This example runs **Grok Build** (`@tanstack/ai-grok-build`) instead of Claude Code.
 * The harness is the only thing that changes — the run-log / WebSocket / tool-bridge
 * topology is adapter-agnostic. Grok Build runs on the Cloudflare sandbox because:
 *   • the adapter delivers the prompt via a file + in-shell stdin-redirection
 *     (`grok … < file`) when the sandbox advertises `writableStdin: false`; and
 *   • `chat()`-provided tools reach grok as a streamable-HTTP MCP server, which is
 ...
 * overrides, but binds NO API key. We extend it with the xAI key the in-sandbox
 * `grok` CLI authenticates with.
 *
 * Accept it under EITHER name for convenience.
 */
export interface AppEnv extends SandboxAgentEnv {
  /** xAI API key. Injected into the sandbox env as XAI_API_KEY. */
  XAI_API_KEY?: string
  /** Alternate name. */
  GROK_API_KEY?: string
}
 ...
 * over the DO-served MCP endpoint — grok sees it as a `tanstack` MCP server tool,
...
export const agent = createCloudflareSandboxAgent<AppEnv>({
  // Grok Build via `@tanstack/ai-grok-build`. The adapter spawns
  // the `grok` CLI inside the sandbox and streams its output back as AG-UI chunks.
  //
  // No special sandboxMode is needed for the xAI Grok Build CLI (unlike Codex's bwrap).
  ...
  // the demo app needs none, so `XAI_API_KEY` here is only for the `grok` CLI.
  ...
  sandbox: (input, env) =>
    defineSandbox({
      ...
      workspace: defineWorkspace({
        // No source to clone — the container image already ships the grok CLI.
        source: { type: 'none' },
        // `grok` authenticates headlessly via XAI_API_KEY. Take it from
        // either `XAI_API_KEY` or `GROK_API_KEY`.
