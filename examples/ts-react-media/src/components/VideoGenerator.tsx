...
function VideoModelCard({
        ...
        generateVideoFn({
          data: {
            ...
            ...(previousJobIdRef.current
              ? { previousJobId: previousJobIdRef.current }
              : {}),
          },
  ...
  const clear = useCallback(() => {
    previousJobIdRef.current = undefined
  ...
  const handleEditVideo = () => {
    ...
    previousJobIdRef.current = result.jobId
    setBilling(undefined)
    setEditPrompt('')
      ...
      {result && !isLoading && !error && (
        <>
          ...
          {(model.provider === 'gemini' ||
            ...
            model.id.startsWith('xai/grok-imagine-video')) && (
            <div className="flex gap-2">
              <input
                type="text"
                value={editPrompt}
