 ...
 *   *    /tool-exec/:runId   → forward to the DO, which EXECUTES the host tool
 *                             the in-container agent called back for.
 *
 * A run is addressed by a DO id derived from its `threadId`, so every event for
 * a conversation lands in one coordinator and the container is reused per thread.
 *
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // We route by the threadId query the tool-exec URL is built with; the DO
    // performs the constant-time token check and runs the real tool.
