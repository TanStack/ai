# Cloudflare sandbox (Codex) — TanStack Start app (Worker + Durable Objects + Container)
...
example with **OpenAI Codex** (`@tanstack/ai-codex`) swapped in for Claude Code —
...
> `wrangler deploy`. Codex **does** run on the Cloudflare sandbox (the adapter
...
handler; the container only runs the `codex` CLI. The whole MCP protocol crosses
...
└───────────────────────────────┬───────────────────────────────────────────┘
                                ...
                                ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  Cloudflare Sandbox (Container)  — the `codex` CLI runs here (Dockerfile).   │
...
1. A DO-backed `ToolBridgeProvisioner` is provided to `chat()`. The codex adapter
   uses it instead of the default `node:http` provisioner — it points codex's MCP
   client at the bridge via `mcp_servers.<name>.url` + `bearer_token` (codex's
...
#    .dev.vars; the factory injects it into the sandbox env for the `codex` CLI.
cd examples/sandbox-cloudflare-codex
cp .dev.vars.example .dev.vars      # then set CODEX_API_KEY (or OPENAI_API_KEY)
...
#    workerd. First run builds the Dockerfile (installs the codex CLI; needs Docker).
...
`wrangler secret put CODEX_API_KEY`. The **agent run** works with no host config —
...
the agent can read. (The demo app the agent builds needs none — `CODEX_API_KEY`
here is only for the `codex` CLI, i.e. the agent itself.)
...
2. **The Cloudflare sandbox has no writable host→process stdin (handled).**
   ...
   `capabilities.writableStdin: false`. The codex adapter normally pipes the
   prompt to `codex exec` over stdin (to keep it out of argv); when it sees
   `writableStdin: false` it instead writes the prompt to a file and redirects
   the CLI's stdin from it **in-shell** (`codex exec … < /tmp/prompt`). The
   redirection happens inside the container, so no host-side stdin write is
   needed and the prompt still never lands in argv. Codex therefore runs on the
   ...
   **Sandbox mode:** this example sets `codexText(…, { sandboxMode: 'danger-full-access' })`.
