...
function toGroqCompatibleSchema(schema: Record<string, any>): Record<string, any> {
  ...
  for (const combiner of ['anyOf', 'oneOf', 'allOf']) {
    if (Array.isArray(result[combiner])) {
      result[combiner] = result[combiner].map((s: any) => toGroqCompatibleSchema(s))
    }
  }

  if (result.$defs) {
    result.$defs = Object.fromEntries(
      Object.entries(result.$defs).map(([k, v]) => [k, toGroqCompatibleSchema(v as any)]),
    )
  }
