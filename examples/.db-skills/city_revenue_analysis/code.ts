...
for (const purchase of purchases) {
  ...
  cityData[city].categoryCounts[category] = (cityData[city].categoryCounts[category] || 0) + purchase.quantity;
}

// Build the result
const result = Object.entries(cityData).map(([city, data]) => {
  // Find most purchased category
  const mostPurchasedCategory = Object.entries(data.categoryCounts)
