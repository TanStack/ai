...
- **New `{ type: 'file' }` arm on `ContentPartSource`**, shaped like the AG-UI `FileSource` arm: `{ type: 'file', value, provider, mimeType? }`. `value` is the opaque handle the provider issued; `provider` names the adapter that issued it. Unlike the AG-UI wire arm, `provider` is required, so a handle can never reach an adapter for a different provider. Each adapter maps `value` to its native wire field: OpenAI (Responses) `input_image`/`input_file` `file_id`, Anthropic `file_id` message source (with the `files-api-2025-04-14` beta), Gemini `fileData.fileUri`, fal storage URL, Grok public URL. `fileSourceFromHandle(handle)` builds the source.
