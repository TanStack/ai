...
The client side needs nothing special — that is the point. Given a route that
...
The other two rows genuinely need the counter — a chunked reply declares
...
one 8 MiB part at a time — flat memory, whatever the artifact's size. The
