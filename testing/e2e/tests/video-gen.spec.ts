...
for (const provider of providersFor('video-gen')) {
  test.describe(`${provider} -- video-gen`, () => {
    test('sse -- generates video via SSE connection', async ({
      ...
      await expect(page.getByTestId('active-run-id')).not.toHaveText('')
      await expect(page.getByTestId('active-status')).toHaveText('generating')
      await expect(page.getByTestId('active-loading')).toHaveText('true')
      await expect(page.getByTestId('final-run-id')).toHaveText('')
