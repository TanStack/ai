...
test.describe('mistral strict tool optional fields', () => {
  test('undoes provider-added nullability before tool validation and execution', async ({
    request,
  }) => {
    const response = await request.post('/api/mistral-strict-tool-null-wire')
    ...
    const tool = result.firstRequestBody.tools.find(
      (entry) => entry.function.name === 'ask_user',
    )
