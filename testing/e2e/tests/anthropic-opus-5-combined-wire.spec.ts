 ...
 *
 * `claude-opus-5` and `claude-opus-5-5` shipped with `supports.tools: []`
 * and were absent from
...
for (const model of ['claude-opus-5', 'claude-opus-5-5']) {
  test.describe(`anthropic — ${model} tools + schema wire format`, () => {
    test('the schema travels in a single streaming request', async ({
      request,
    }) => {
      const { capturedRequests } = await runRoute(request, model)
    ...
    test('the web_search tool rides along and no structured_output tool appears', async ({
      ...
      expect(toolNames(capturedRequests[0]?.body ?? null)).toContain(
        'web_search',
      )
