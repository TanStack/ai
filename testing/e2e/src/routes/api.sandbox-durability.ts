 ...
 *
 * Proves that `withSandboxPersistence` makes sandbox resume durable ACROSS
 * independent runs: each POST is a fresh `chat()` with a brand-new middleware
 * context, and the ONLY shared state is a module-singleton `SandboxStore` (an
 * in-memory store standing in for a durable backend — the durable backends
 * themselves are covered by the per-backend conformance suites). A second run
 * for the same `threadId` must therefore RESUME the sandbox the first run
 * created, not create a second one.
 *
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const record = await store.get(sandbox.key({ threadId, runId }))
